<template class="bg-brown">
    <div class="flex justify-between w-card-guia mb-5 space-x-3" v-if="isLoaded">
        <div class="w-full rounded-xl shadow-inner shadow-white-op-25 bg-preto-ebano h-20"  v-for="(job, index) in filteredBP.slice(0,3)" :key="job.id">
            <div class=" flex flex-wrap justify-center w-full p-3 top-3">
                ...
                <span class="flex items-center w-6/12 mt-2 ml-3 text-sm justify-center" :class="{ 'text-gold-rk': index === 0, 'text-silver-rk': index === 1, 'text-bronze-rk': index === 2}">{{ job.Name }}</span>
                <!-- <span class="flex items-center w-full sm:w-6/12 text-md justify-center">BattlePoint: {{ job.BattlePoint }}</span> -->
                <span class="flex items-center w-full sm:w-12/12 text-sm justify-center">RankPoint: {{ job.RankPoint }}</span>
            </div>
        </div>
    </div>
    <div class="shadow-inner shadow-white-op-25 bg-preto-ebano w-card-guia h-auto rounded-50 overflow-y-auto table-container" v-if="isLoaded">
        <table  class="table-auto ml-5 mr-5 w-11/12 items-center mt-5 mb-2 max-h-40 z-50 ">
            ...
            <tbody class="xl:mt-4 lg:mt-0">
                <tr class="text-center hover:bg-yellow cursor-default lg:text-sm" v-for="(job, index) in filteredBP.slice(3,20)" :key="job.id">
                    <td>{{ index+4 }}</td>
                    <td>{{ job.Name }}</td>
