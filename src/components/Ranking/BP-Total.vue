<template class="bg-brown">
        <div class="flex justify-between w-card-guia mb-10 space-x-3" v-if="isLoaded">
            <div class="w-full rounded-xl shadow-inner shadow-white-op-25 bg-preto-ebano h-20"  v-for="(job, index) in filteredBP.slice(0,3)" :key="job.id">
                ...
                <span class="flex items-center w-full sm:w-12/12 text-md justify-center">BattlePoint: {{ job.BattlePoint }}</span>
                <!-- <span class="flex items-center w-full sm:w-6/12 text-md justify-center">RankPoint: {{ job.RankPoint }}</span> -->
            </div>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            filteredBP() {
                return this.jobs.filter(job => job.BattlePoint > 0).sort((a, b) => b.BattlePoint - a.BattlePoint);
            }
        }
    }
</script>
